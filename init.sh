...
build_binutils() {
    echo "tools/bin/ is missing binutils - building binutils for x86_64-elf"
    cd $cc_PREFIX/build/
...
build_gcc() {
    echo "tools/bin/ is missing gcc - building gcc for x86_64-elf"
    cd $cc_PREFIX/build/
    mkdir -p build_gcc
    ...
    # Build GCC
    echo "Building GCC..."
    make all-gcc -j8 > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "Error: Building GCC failed!"
        exit 1
    fi

    make all-target-libgcc -j8 > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "Error: Building target libgcc failed!"
        exit 1
    fi

    make all-target-libstdc++-v3 -j8 > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "Error: Building target libstdc++ failed!"
        exit 1
    fi

    # Install GCC
    echo "Installing GCC..."
    make install-gcc -j8 > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "Error: Installing GCC failed!"
        exit 1
    fi

    make install-target-libgcc -j8 > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "Error: Installing target libgcc failed!"
        exit 1
    fi

    make install-target-libstdc++-v3 -j8 > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "Error: Installing target libgcc failed!"
        exit 1
    fi
